...
sed -i -e "s/ORGDASHREPO/$(echo $CI_REPO | sed 's/\//-/g')/g" rancher-compose.yml docker-compose.yml

# set the correct organisation/repo name in the compose files with slash (using : as delimiter as variable contains slashes)
sed -i -e "s:ORGSLASHREPO:$CI_REPO:g" rancher-compose.yml docker-compose.yml
